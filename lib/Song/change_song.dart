...
class ChangeSongName extends StatefulWidget {
  const ChangeSongName(this.post,{super.key});

  final Post post;//Postクラスのインスタンスを受け取る

  @override
  State<ChangeSongName> createState() => ChangeSongNameState();
}


class ChangeSongNameState extends State<ChangeSongName> {
  ...
  String updatedWord = '';//ユーザーが入力した新しい文字列を格納する変数
    ...
    await FirebaseFirestore.instance.collection("Song").doc(widget.post.id).update({//ドキュメントIDを使用して、特定のドキュメントを参照
      'name': 'Flutter',
      'text': updatedWord,
